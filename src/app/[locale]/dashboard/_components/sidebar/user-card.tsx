...
export function UserCard({ user }: { user?: User }) {
  ...
  return (
    <>
      {/* Separator */}
      <div className="w-full h-px bg-[#E5E5E5] mb-1"></div>
      <Card className="flex w-full items-center justify-between gap-3 px-2 py-3 shadow-none">
        {/* User Info Section */}
        <div className="flex items-center gap-3">
          <Avatar className="h-10 w-10">
            ...
            <AvatarFallback
              style={{
                backgroundColor: `hsl(${(first.charCodeAt(0) + last.charCodeAt(0) * 7) % 360}, 70%, 80%)`,
                color: '#111',
                fontWeight: 600,
              }}
            >
