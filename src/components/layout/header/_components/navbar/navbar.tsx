...
export default function Navbar() {
  ...
  return (
    <nav className="bg-maroon-700 dark:bg-softPink-200 relative">
      <div className="flex items-center justify-between lg:justify-center px-4 py-2 lg:py-0">
        {/* Mobile Menu Button */}
        ...
        {/* Desktop Navigation */}
        <ul className="hidden lg:flex justify-center items-center gap-2 xl:gap-4">
          {Navlinks.map(({ name, href, icon: Icon }) => (
            <li key={name}>
              <Link
                href={href}
                className="flex items-center gap-1.5 xl:gap-2 text-zinc-50 dark:text-zinc-800 
                           text-sm xl:text-base font-medium py-3.5 px-2 xl:px-3 capitalize 
