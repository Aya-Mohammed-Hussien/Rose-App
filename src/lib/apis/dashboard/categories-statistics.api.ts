import { CategoriesStatisticsResponse } from '@/lib/types/statistics';
import { getToken } from '@/lib/utils/get-token.util';

/**
 * Fetch categories statistics from the backend API.
 * This function retrieves the user token using getToken() and
 * sends it as a Bearer token in the Authorization header.
 */

export const getCategoriesStatistics = async (): Promise<CategoriesStatisticsResponse> => {
  try {
    // Retrieve the JWT token from cookies
    const token = await getToken();

    // Send a GET request to the /categories endpoint
    const response = await fetch(`${process.env.NEXT_PUBLIC_API}/statistics/categories`, {
      method: 'GET',
      headers: {
        // Include the token in the Authorization header
        Authorization: `Bearer ${token}`,
      },
    });

    // Check if the response status is not OK (e.g., 401)
    if (!response.ok) {
      throw new Error(`Failed to get categories statistics: ${response.status}`);
    }

    // Parse the response body as JSON
    const payload: CategoriesStatisticsResponse = await response.json();

    // Return the parsed data
    return payload;
    
  } catch (error) {
    // Rethrow the error so it can be handled in the calling code
    throw error;
  }
};
